...
bool test_append_array_aux(std::ostream &out,
  ...
  out << ": got ";
  if (! success && reason > 1) {
    if (reason == 2) out << current_size << " as wrong array size ";
    else if (reason == 3) out << max_size << " as wrong array max_size ";
    else if (reason == 4) out << max_size << " as wrong array max_size ";
    out << "and ";
  }
...
bool test_remove_from_array_aux(std::ostream &out,
  ...
  out << ": got ";
  if (! success && reason > 1) {
    if (reason == 2) out << current_size << " as the wrong array size ";
    else if (reason == 3) out << max_size << " as the wrong array max_size ";
    else if (reason == 4) out << max_size << " as the wrong array max_size ";
    out << "and ";
  }
  if (copy == NULL) out << "NULL ";
  else print_array(out, copy, current_size);
...
int test_shrink_array(std::ostream &out, const std::string test_name) {
    ...
    std::vector<int> res = {
      test_extend_array_aux(out, "shrink_array", shrink_array,
                            t1, 5, 2, "[1,2,3,4,5]", 5, 2),
      test_extend_array_aux(out, "shrink_array", shrink_array,
                            t1, 5, 5, "[1,2,3,4,5]", 5, 5),
      test_extend_array_aux(out, "shrink_array", shrink_array,
                            t1, 5, 0, "[1,2,3,4,5]", 5, 0),
      test_extend_array_aux(out, "shrink_array", shrink_array,
                            t1, 0, 0, "[]", 0, 0)
    };
...
int test_remove_from_array(std::ostream &out, const std::string test_name) {
    ...
    std::vector<int> res = {
      ...
      test_remove_from_array_aux(out, fun_name, remove_from_array,
                                 t1, 1, 5,
                                 "[0]", 1, 5)
      // [SM] Corner case when array is empty was ambiguous in the description.
      // Just remove these test cases
      // test_remove_from_array_aux(out, fun_name, remove_from_array,
      //                            t1, 0, 3,
      //                            "[]", 0, 3),
      // test_remove_from_array_aux(out, fun_name, remove_from_array,
...
bool test_simulate_projectile_aux(std::ostream &out,
  ...
  print_tested_function(out, "simulate_projectile", magnitude, angle, simulation_interval,
                        targets_repr, tot_targets, obstacles_repr, tot_obstacles);
...
bool test_merge_telemetry_aux(std::ostream &out,
  ...
  std::string telemetries_sizes_repr = get_array_str(telemetries_sizes, tot_telemetries);

  {
    ...
    for (int i = 0; i < tot_telemetries; i++) {
      ...
      std::string tel = get_array_str(telemetries[i], telemetries_sizes[i]);
